...
<?php
    ...
    {
        if(!empty($_POST['name']) && !empty($_POST['address']) && !empty($_POST['branch']) 
            && !empty($_POST['bg']) && !empty($_POST['phone']) && !empty($_POST['email'])) 
        {
            $name = $_POST['name'];
            ...
            $query = "INSERT INTO donor(d_id, b_id, d_name, address, donated_item, phone, email) VALUES 
            (donor_seq.nextval, $branch, '$name', '$address', '$bg', '$phone', '$email')";
